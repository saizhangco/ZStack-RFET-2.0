...
void LCD_P32x64Ch(unsigned char x,unsigned char N);
#endif //LCD
...
/****************功能描述：显示32*64点阵  显示的坐标（x,y），y为页范围0~7*******************************/
void LCD_P32x64Ch(unsigned char x,unsigned char N)
{
    unsigned char x_count = 0;
    unsigned char y_count = 0;
    unsigned int addr = 256*N;// 256*N
    for(y_count=0;y_count<8;y_count++)
    {
        LCD_Set_Pos(32*x,y_count);
        for( x_count=0;x_count<32;x_count++ )
        {
            LCD_WrDat(F32X64[addr]);    
            addr += 1;
        }
    }
}
...
/****************************与业务逻辑相关的部分*******************************************************/
void TakeMedicine(unsigned char amount)
{
    unsigned char num1 = amount%100/10;
    unsigned char num2 = amount%10;
    LCD_P32x64Ch(0,10);
    LCD_P32x64Ch(1,13);
    LCD_P32x64Ch(2,num1);
    LCD_P32x64Ch(3,num2);
...
void AddMedicine(unsigned char amount)
{
    unsigned char num1 = amount%100/10;
    unsigned char num2 = amount%10;
    LCD_P32x64Ch(0,11);
...
void QueryMedicine(unsigned char amount)
{
    unsigned char num1 = amount%100/10;
    unsigned char num2 = amount%10;
    LCD_P32x64Ch(0,12);
