...
uint8 test_lcd_func = 0;
uint8 query_medicine = 0;


/*********************************************************************
 * LOCAL FUNCTIONS
 */
void SampleApp_LcdReset(void);
...
void SampleApp_ProcessUartData(mtOSALSerialData_t *MSGpkt); //串口处理函数
void SampleApp_Send_C2E_Message( uint8* msg );              //通过无线发送数据到终端设备
...
uint16 SampleApp_ProcessEvent( uint8 task_id, uint16 events )
{
	...
	if( events & SAMPLEAPP_ED_LCD_RST_EVT )
	{
		SampleApp_LcdReset();	// 重置LCD屏幕
		return ( events ^ SAMPLEAPP_ED_LCD_RST_EVT );
	}
#if ( ZSTACK_DEVICE_BUILD == DEVICE_BUILD_ENDDEVICE )	//终端设备
    //^^^^^^^^触发按键确认事件^^^^^^^^
    ...
    if( events & SAMPLEAPP_KEY_ACK_EVT)//SAMPLEAPP_KEY_QUERY_EVT )
    {
      ...
      else if( test_lcd_func == 1 )
      {
          AddMedicine(34);
          test_lcd_func++;
      }
      else if( test_lcd_func == 2 )
      {
          QueryMedicine(78);
          test_lcd_func++;
      }
      else if( test_lcd_func == 3 )
      {
          MedicineBusinessEnd();
          test_lcd_func = 0;
      }
...
void SampleApp_LcdReset(void)
{
    if(SampleAppAckEvent == NOT_EXIST && query_medicine == 0 )
    {
        MedicineBusinessEnd();  //关闭LCD上的显示
    }
}
            ...
            */
            ...
            else if( format == 0x00 )
            {
                  ...
                  )
                {
                    if( 0x00 == SampleUtil_CompareUint8Array(localEndDevMsg.command,"TKME",4))
                    {
                        // 清除一个BUG：当电子标签出于查询状态下，开始取药，重置（取药标志位query_medicine）
                        query_medicine = 0;
                        TakeMedicine(localEndDevMsg.value.number);
                    }
                    else if( 0x00 == SampleUtil_CompareUint8Array(localEndDevMsg.command,"ADME",4))
                    {
                        // 清除一个BUG：当电子标签出于查询状态下，开始补药，重置（取药标志位query_medicine）
                        query_medicine = 0;
                        AddMedicine(localEndDevMsg.value.number);
                    }
                    else if( 0x00 == SampleUtil_CompareUint8Array(localEndDevMsg.command,"LTME",4))
                    {
                        QueryMedicine(localEndDevMsg.value.number);
                    }
                    /*
...
void SampleApp_Send_E2C_Ack( uint8* value, uint8 length )
{
    ...
    SampleApp_P2P_DstAddr.addr.shortAddr = 0x0000;//指明发给协调器
    // guid
    for( i=0;i<4;i++ )
    {
...
/*********************************************************************
 * @fn      SampleApp_Send_E2C_PushBtn_Evt
 *
 * @brief   终端设备主动发送按钮触发消息
 *
 * @param   none
 *
 * @return  none
